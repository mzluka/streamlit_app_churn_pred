...
""",
    ...
    if CreditScore > 100 and EstimatedSalary > 5000 and IsActiveMember == 1 and NumOfProducts > 1 and Age < 60 and Tenure > 3 and Balance > 500:
            st.success('Вероятность оттока составляет менее 30%.')
            st.markdown(churn_html, unsafe_allow_html= True)
  
    if CreditScore > 200 and EstimatedSalary > 10000 and IsActiveMember == 1 and NumOfProducts > 2 and Age < 50 and Tenure > 3 and Balance > 1000:
            st.success('Вероятность оттока составляет менее 20%.')
            st.markdown(churn_html, unsafe_allow_html= True)

    if CreditScore > 300 and EstimatedSalary > 15000 and IsActiveMember == 1 and NumOfProducts > 2 and Age < 40 and Tenure > 3 and Balance > 2000:
